...
import { parseFileContent, parseSide, parseTimestamp } from '../src/Engine/csv-parser';
import { compareTransactions, reconcileTransactions } from '../src/Engine/match-engine';
import { ManualResolution, NormalizedTransaction } from '../src/Types/reconciliation-types';
...
C-9001,2025-07-06 11:20:00,BTC-USD,B,0.15,63100,9465.00,SETTLED`;
  ...
  describe('Comparison Logic Tests', () => {
    const baseInternal: NormalizedTransaction = {
      id: 'int-1',
      source: 'INTERNAL',
    ...
    it('flags identical transactions as EXACT_MATCH', () => {
      const external: NormalizedTransaction = { ...baseInternal, source: 'EXTERNAL', id: 'ext-1' };
      const result = compareTransactions(baseInternal, external);
      expect(result.status).toBe('EXACT_MATCH');
      expect(result.diffs).toHaveLength(0);
    });

    it('flags minor amount differences as TOLERATED_MATCH', () => {
      const external: NormalizedTransaction = {
        ...baseInternal,
        source: 'EXTERNAL',
        id: 'ext-1',
    ...
    it('flags 25 minute timestamp drift as TOLERATED_MATCH', () => {
      const external: NormalizedTransaction = {
        ...
        source: 'EXTERNAL',
        id: 'ext-1',
        tradedAt: '2025-07-01T09:40:00.000Z',
      };
      const result = compareTransactions(baseInternal, external, { amountTolerance: 0.05, timestampToleranceSeconds: 3600 });
      expect(result.status).toBe('TOLERATED_MATCH');
      expect(result.diffs).toHaveLength(1);
...
C-9001,2025-07-06 11:20:00,BTC-USD,B,0.15,63100,9465.00,SETTLED`;
    ...
    it('persists manual resolutions over subsequent runs', () => {
      const internalRecords: NormalizedTransaction[] = [
        {
          id: 'int-1',
          source: 'INTERNAL',
      ...
      const externalRecords: NormalizedTransaction[] = [
        {
          id: 'ext-1',
          source: 'EXTERNAL',
